...
const createParentDirectoy = (directories, path) => {
  if (!directories.length) return;
  const currentPath = path + "/" + directories[0];
  if (fs.existsSync(currentPath) === false) {
    fs.mkdirSync(currentPath);
    console.log("create parent directory success : ", directories[0]);
    path = currentPath;
  }
  directories.shift();
  createParentDirectoy(directories, path);
};
...
export async function generateModule() {
  ...
  const originDirectory = "/src/modules";

  const parentDirectories = originDirectory
    .split("/")
    .filter((directory) => directory !== "");

  const serverPath = path.join(__dirname, "..");
  createParentDirectoy(parentDirectories, serverPath);
